...
[Index(nameof(CalendarId), IsUnique = true)]
public class Member
{
    /// <summary>
    ...
    /// guarding the feed, so it is always generated with <see cref="Guid.NewGuid"/> and never derived
    /// from <see cref="Id"/>. It defaults to a freshly generated value so that a member can never be
    /// persisted with a guessable identifier, and it can be regenerated by the member to revoke
    /// a leaked feed URL.
    /// </summary>
    [JsonIgnore] public Guid CalendarId { get; set; } = Guid.NewGuid();
