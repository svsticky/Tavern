    ...
    ILogger<MailSubscriptionOutboxWorker> logger) : BackgroundService
{
    ...
    private async Task<bool> TryProcessNextTaskAsync(CancellationToken ct)
    {
        ...
        catch (NonRetriableMailSubscriptionException ex)
        {
            // Retrying can never succeed (e.g. the member deleted their account, so their email was
            // permanently forgotten by the provider) - discard rather than retry forever.
            logger.LogWarning(ex, "Discarding mail subscription outbox task for email {Email} - not retriable.", task.Email);
            db.MailSubscriptionOutboxTasks.Remove(task);
        }
        catch (Exception ex)
        {
