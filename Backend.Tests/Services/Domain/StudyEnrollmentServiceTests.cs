...
public class StudyEnrollmentServiceTests : IDisposable
{
    private readonly DbContextOptions<PostgresDbContext> _dbOptions;
    private readonly PostgresDbContext _db;
    private readonly IPermissionService _permissionService;
    private readonly StudyEnrollmentService _service;
    private readonly Guid _userId = Guid.NewGuid();

    public StudyEnrollmentServiceTests()
    {
        ...
        _service = new StudyEnrollmentService(
            _db,
            _permissionService,
            NullLogger<StudyEnrollmentService>.Instance
        );
    }
    ...
    public async Task GetStudyEnrollments_UserNotBoard_ThrowsUnauthorized()
    {
        ...
        await Assert.ThrowsAsync<UnauthorizedAccessException>(() =>
            _service.GetStudyEnrollments(dto, _userId, CancellationToken.None));
    }
    ...
    public async Task GetStudyEnrollments_BoardUser_ReturnsEnrollments()
    {
        ...
        // Act
        var result = await _service.GetStudyEnrollments(dto, _userId, CancellationToken.None);
    ...
    {
        // Act
        var result = await _service.GetStudyEnrollment(999u, _userId, CancellationToken.None);
    ...
    public async Task GetStudyEnrollment_OwnEnrollment_ReturnsEnrollmentWithoutPermissionCheck()
    {
        ...
        // Act
        var result = await _service.GetStudyEnrollment(2u, member.Id, CancellationToken.None);
    ...
    public async Task GetStudyEnrollment_OtherUserEnrollment_EnsuresBoardMember()
    {
        ...
        // Act
        var result = await _service.GetStudyEnrollment(3u, _userId, CancellationToken.None);
    ...
    public async Task CreateStudyEnrollment_MemberNotFound_ThrowsException()
    {
        ...
        await Assert.ThrowsAsync<Exception>(() =>
            _service.CreateStudyEnrollment(dto, _userId, CancellationToken.None));
    }
    ...
    public async Task CreateStudyEnrollment_OwnUserNotBoard_DoesNotRequireBoardPermission()
    {
        ...
        // Act
        var result = await _service.CreateStudyEnrollment(dto, member.Id, CancellationToken.None);
    ...
    public async Task DeleteStudyEnrollment_NotFound_ThrowsException()
    {
        // Act & Assert
        await Assert.ThrowsAsync<Exception>(() =>
            _service.DeleteStudyEnrollment(999u, _userId, CancellationToken.None));
    }
    ...
    public async Task DeleteStudyEnrollment_Exists_DeletesFromDatabase()
    {
        ...
        // Act
        await _service.DeleteStudyEnrollment(5u, _userId, CancellationToken.None);
    ...
    public async Task PatchStudyEnrollment_NullPatch_ThrowsArgumentException()
    {
        ...
        await Assert.ThrowsAsync<ArgumentException>(() =>
            _service.PatchStudyEnrollment(1u, null!, _userId, CancellationToken.None));
    }
    ...
    public async Task PatchStudyEnrollment_ModifiesRestrictedFields_ThrowsArgumentException()
    {
        ...
        // Act & Assert
        await Assert.ThrowsAsync<ArgumentException>(() =>
            _service.PatchStudyEnrollment(1u, patchDoc, _userId, CancellationToken.None));
    }
    ...
    public async Task PatchStudyEnrollment_DifferentUserAndDurationPassed_DoesNotEnsureBoard()
    {
        ...
        // Act
        await _service.PatchStudyEnrollment(10u, patchDoc, member.Id, CancellationToken.None);
    ...
    public async Task PatchStudyEnrollment_OwnUserButDurationNotPassed_EnsuresBoard()
    {
        ...
        // Act
        await _service.PatchStudyEnrollment(11u, patchDoc, member.Id, CancellationToken.None);
