...
as a download, because the URL is a live subscription and not a file; and every activity is currently emitted as a
timed UTC event, since whole-day events (`VALUE=DATE`) are not implemented yet.

Whole-day support is pending because an activity's wall clock cannot be recovered reliably today. The frontend
converts the entered `datetime-local` value using the *browser's* timezone, so a `00:00`–`23:59` activity only round-trips
back to `00:00`–`23:59` when its creator happened to be on the association's clock. The `AssociationTimeZone` environment
variable (`Europe/Amsterdam` by default) is the intended anchor for that conversion: once the frontend anchors entry to it
as well, anything reasoning about the *day* or *time-of-day* a member entered must go through it rather than reading the
UTC value directly.
