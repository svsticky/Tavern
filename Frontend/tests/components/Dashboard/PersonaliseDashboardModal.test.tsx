...
describe("PersonaliseDashboardModal", () => {
  ...
  it("allows switching a widget from sidebar to main", () => {
    renderWithProviders(<PersonaliseDashboardModal {...defaultProps} />);

    const moveToMainButtons = screen.getAllByRole("button", {
      name: "move_to_main",
    });
    fireEvent.click(moveToMainButtons[0]); // Move first sidebar widget (my_enrollments) to main

    fireEvent.click(screen.getByRole("button", { name: "done" }));

    const savedWidgets: DashboardWidgetConfig[] = mockOnSave.mock.calls[0][0];
    const enrollments = savedWidgets.find((w) => w.id === "my_enrollments");
    expect(enrollments?.column).toBe("main");
  });

  it("allows moving a widget up and down in main and sidebar, and toggling sidebar widget", () => {
    renderWithProviders(<PersonaliseDashboardModal {...defaultProps} />);

    // Toggle sidebar widget visibility
    const enrollmentsCheckbox = screen.getByRole("checkbox", {
      name: /my_enrollments/i,
    });
    fireEvent.click(enrollmentsCheckbox);
    expect(enrollmentsCheckbox).not.toBeChecked();

    // Move up in main (second item: upcoming_activities)
    const moveUpButtons = screen.getAllByRole("button", { name: "move_up" });
    fireEvent.click(moveUpButtons[1]); // upcoming_activities up

    // Move down in sidebar (first item: my_enrollments)
    const moveDownButtons = screen.getAllByRole("button", {
      name: "move_down",
    });
    fireEvent.click(moveDownButtons[2]); // first item in sidebar

    // Move up in sidebar (second item in sidebar)
    const moveUpButtonsUpdated = screen.getAllByRole("button", {
      name: "move_up",
    });
    fireEvent.click(moveUpButtonsUpdated[3]); // second item in sidebar up
