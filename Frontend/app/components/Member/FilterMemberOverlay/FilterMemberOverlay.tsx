...
import {
  ...
  handleResetFilters,
  loadStudies,
} from "./FilterMemberOverlay.handlers";
 ...
 * ```
 */
export default function FilterMemberOverlay({
  filters,
  onFilter,
}: {
  ...
  const [study, setStudy] = useState<Study | null>(
    filters?.studyId ? { id: filters.studyId, title: "" } : null,
  );
  const [gratie, setGratie] = useState<boolean | null>(filters?.gratie ?? null);
  const [lidVanVerdienste, setLidVanVerdienste] = useState<boolean | null>(
    filters?.lidVanVerdienste ?? null,
  );
  const [ereLid, setEreLid] = useState<boolean | null>(filters?.ereLid ?? null);
  const [begunstiger, setBegunstiger] = useState<boolean | null>(
    filters?.begunstiger ?? null,
  );
  const [suspended, setSuspended] = useState<boolean | null>(
    filters?.suspended ?? null,
  );
  const [inactive, setInactive] = useState<boolean | null>(
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 gap-3">
        ...
        <Select
          ...
          onChange={(e) =>
            setStudy(
              studies.find((s) => s.id === Number(e.target.value)) || null,
            )
          }
          options={[
            { value: "", label: t("all_studies") },
        ...
        <TriStateFilter
          ...
          value={lidVanVerdienste}
          onChange={setLidVanVerdienste}
        />
        <TriStateFilter
          label={t("ere_lid")}
          value={ereLid}
