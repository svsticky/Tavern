import { Calendar, ImageIcon, MapPin, UsersRound, PencilIcon } from "lucide-react"; // PencilIcon toegevoegd
      ...
      >
        ...
        <div className="relative aspect-[1/1.414] w-full overflow-hidden bg-gray-100">
          ...
          {!hasPoster && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-200">
              ...
              <span className="text-sm font-medium text-slate-400">{t("no_poster")}</span>
            </div>
          )}
        ...
        <div className="rounded-b-2xl border border-t-0 border-gray-200 p-3 bg-white">
          ...
          <div className="mt-0 flex flex-col text-[14px] text-gray-500">
            <div>
              <Calendar size={12} />
              {startDate.getDate()} {formatDate(startDate, "monthShort")} • {formatDate(startDate, "timeOnly")}
            ...
            <div className="mt-1 flex items-center gap-1.5">
              ...
              {activity.participantLimit 
                ...
                : (activity.enrollments?.filter((e) => !e.isOnWaitingList).length ?? 0) + " " + t("participants")}
            </div>
          </div>
        </div>
      </Tile>
