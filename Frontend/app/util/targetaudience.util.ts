...
export const isMemberInTargetAudience = (
  ...
  const audienceMask = typeof targetAudience === 'number' 
    ? targetAudience 
  ...
  const activeEnrollments = member.studyEnrollments.filter(se => se.status === 'Enrolled');

  // 1. First Years
  if (audienceMask & AudienceFlags.FirstYears) {
    if (activeEnrollments.some(se => 
